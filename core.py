...
class DataBuilder(data.Dataset):
        ...
        def __getitem__(self, index):
                ...
                return self.configuration['decoder'](datum, self.configuration['transform'])
...
def mapping_decoder(datum, t):
    ...
    sample = {"FaceA": iAf, "FaceB": iBf, "BodyA":iAb, "BodyB":iBb, "Whole":iW, 'Label': label, "Subset" : true_label}
...
configurations['train']['sampler'] = {'bs':32,'sampler':train_sampler} # train sampler and subset are mutually exclusive
configurations['val']['decoder'] = mapping_decoder
configurations['val']['sampler'] = {'bs':32,'shuffle':False}
configurations['test']['decoder'] = mapping_decoder
configurations['test']['sampler'] = {'bs':32,'shuffle':False}
...
V = VAE_Building(model_choice="FaceVAE Zodiac", dbs={'train':db_train, 'val':db_val, 'test':db_test}, result_path="/scratch/Jack/research lab/True_Relations/")
V.train()

# From Mixture of concatenation, predict relationship (Loss 2)
# Incur loss for class distribution in each mixture (Loss 3) (Last)

# Entire Dataset, Balanced Cluster Loss
# Michael cluster loss
# Clustering
...
# Leaderboard:
# Best Test on 4-stream should be 57.2%, while 2-stream should be 44.8% (attributes though)
